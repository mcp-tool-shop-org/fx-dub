...
**v1.1.1 版本——已完成整个流程，两个测试结果均为“通过”，并且画面与声音同步。** 在容器测试中，一段包含两个角色的夜间街道场景获得了 **19/19** 的高分（48 kHz，-18.09 LUFS，对话音量为 +11.17 LU，背景音乐在床边播放，共 161 帧，所有内容完整，时长 10.069 秒），而在内容测试中获得了 **11/11** 的高分。与声音同步的版本保持了相同的参数——832 × 480 分辨率，161 帧，两个音轨——其中 MAC 在说台词时嘴巴张开，而画面外角色的台词则在 MAC 不说话时播放。

当你通过 `--scene` 后，它会获得 **19/20** 分，并且失败是真实的：已交付的运行中的字幕声称在一个只有一个人的场景中出现了两个人。此检查是在此版本中新增的，并且它发现了一个之前一直显示为绿色的缺陷。进行了 189 次测试，CI 显示为绿色。完整历史记录在 [CHANGELOG](CHANGELOG.md) 中。
