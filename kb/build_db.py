...
NEXT_ACTIONS = [
    ...
    ("Director to choose footsteps take A (leather heels) or B (heavy boots) - both corrected to wet concrete and sent 2026-08-22. Re-mix is free and byte-exact via LoadAudio storage keys; no regeneration needed.", "Director", "open"),
    ("Promote v2.3 into a SAVED cloud graph: it currently exists only as hand-authored API JSON. Rebuild it via save_workflow so it is a durable tab, and archive the pulled JSON to workflows/comfy-cloud/as-built/fx-dub-v2.3.json.", "advisor session", "open"),
    ("TEST THE VOICE-CLONING PATH now that LoadAudio is proven: put a reference voice clip into the asset store, take its storage key from get_output, and feed FB_Qwen3TTSVoiceClonePrompt / FL_ChatterboxTTS.audio_prompt. This would retire the cloning-is-local-only constraint entirely.", "advisor session", "open"),
    ("DONE 2026-08-22: audition_receipt.py corrected. dialogue_anchored now computes VO-minus-BED from two stem meters (a bed_lufs manifest is REQUIRED; absent it the verdict is an honest FAIL, never a pass by omission). Source stems check native_rate instead of rate48k, and rates_agree considers only delivered masters. 102 tests green; the falsifiability gate forced red cases for every new check.", "advisor session", "done"),
