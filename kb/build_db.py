...
NEXT_ACTIONS = [
    ...
    ('BUILD fx-dub v2.3 from the bake-off winners once the Director picks: (1) an ElevenLabs eleven_sfx_v2 bed replacing the ACE-Step bed, (2) the chosen voice, (3) bed on AudioMix.audio_1 to hold 48 kHz, (4) a THIRD AudioLoudnessMeter on the bed stem, (5) bed gain set from the MEASURED bed-vs-VO delta rather than a fixed -15, (6) the caption SaveText fixed. Author it directly with submit_workflow - no agent round trip needed.', 'advisor session', 'open'),
    ('Fix tools/audition_receipt.py: loudness:dialogue_anchored compares two INTEGRATED LUFS values and is structurally incapable of measuring ducking depth (the bed is gated out) - re-point it at a bed-stem meter once v2.3 emits one. And audio:stem_vo:rate48k can NEVER pass: all cloud TTS is 24 kHz, so that check is a false alarm by construction.', 'advisor session', 'open'),
