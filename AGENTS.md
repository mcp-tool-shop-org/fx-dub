...
**State snapshot: 2026-09-14, session 6 — v1.2.0 IS SHIPPED to PyPI.** The content verifier's core is exported as `fxdub.verify` and `dialogue_receipt` is reimplemented on top of it, so the surface third parties build on is the one this repo's own CLI exercises. **284 tests.** The full treatment's Phase-0 identity gate then caught a committed receipt carrying the operator's absolute path (public repo since v1.0.0; published artifacts were always clean), and the tag/main CI double-fire was closed at the trigger. Both have detectors proven red against the real pre-fix bytes. v1.2.0 published via Trusted Publishing; the re-fetched PyPI wheel and sdist both scan RESULT CLEAN. *(Session 4: the dub is delivered — v2.8 scores 19/19 with Director-approved voices.)*
