...
- **Live thread:** round-6 brief awaits relay — GO on pinning `VHS_VideoCombine.frame_rate` to a `PrimitiveFloat` 16.0 (fixture-specific; general runs go back to `source_fps` once that derivation is measured). Then: Director uploads the clip via the **browser** (no API route exists — `upload_file` is image-only), we send the agent the exact filename, it re-validates, and only then is the audition ordered.
- **Audition fixture:** the Wan rain/cyborg clip — 161 frames, 10.062 s, ~16 fps, silent, `sha256 9985a8ba…`, cross-verified by two independent probes; staged at `C:/Users/mikey/Downloads/fxdub-audition-clip.mp4`. **Do not regenerate test footage**: those two video jobs cost ~180 credits versus ~10–15 for a whole audio run.
