...
## パブリック API — `fxdub.verify`

**v1.2.0 での新機能。** 2 つのコンソールスクリプトは、*ビデオの吹き替え*を検証します。それらの基盤となるコア（スクリプトと実際に発せられたセリフを照合する）は、ビデオに固有のものではなく、他のツールが内部をインポートする代わりに、その上に構築できるように、宣言された安定したインターフェースとしてエクスポートされます。
...
これらの3つの要素は、作成されたテキストを生成された音声に変換するあらゆるパイプラインに適用されます。**スクリプトで要求されたセリフは発せられなければならず、要求されていないセリフは発せられてはならず、キャラクターは単一の声で表現されなければなりません。** これらは、宣言される前に別のコードベースに対して検証されました。EPUB→オーディオブックレンダラーで測定された4つのコンテンツの欠陥（注釈付きの脚注、部分的なレンダリングによって削除され、成功として報告されたチャプター、1つの声に統合された3つのキャラクター、散文として読み上げられたマークアップ）は、[`tests/test_verify.py`](tests/test_verify.py)に記録されており、何も報告しないクリーンなコントロールと並んで存在します。

API には意図的に**含まれない**もの：文字起こし（fx-dub は与えられた単語のタイミングを使用するため、依存関係はありません）、しきい値（それらはすべて、あるメディアで耳で検出された欠陥に由来するため、ポリシーを所有する呼び出し側に属します）、およびメディア固有のチェック（オーバーラップ、行の途中でのずれ、クリップの適合、ダッキングの深さ、キャプションとキャストの比較）。意味のある結果をもたらさないメディアでのチェックは、チェックがないよりも悪い結果になる可能性があります。なぜなら、それは成功として解釈されるからです。

`fxdub-dialogue` は、この API の消費者でもあります。コンテンツの検証結果を `verify` を通じて計算し、レシートを *吹き替え* レシートにするために必要なものだけを追加します。これは、`verify` を壊し、CLI がそれに合わせて動作を停止させるテストによって強制されます。そのため、パブリックパスが静かに誰も使用しないものになることはありません。
...
**v1.2.0 — パイプラインが提供され、両方のレシートが正常、映像はリップシンクされ、アライメントコアは現在、宣言されたパブリック API です。** 2つのキャラクターが登場する夜の街のシーンは、コンテナコントラクト（48 kHz、-18.09 LUFS、ダイアログ +11.17 LU、バックグラウンド、161フレーム、10.069秒）で **19/19** 点、コンテンツコントラクトで **11/11** 点を獲得します。リップシンクされたバージョンは、MAC の口が自分のセリフを言うときに開いており、フレーム外のキャラクターが話しているときは閉じている状態で、832 × 480、161フレーム、両方のトラックという同じコントラクトを維持します。

`--scene` を通過させると、**19/20** 点を獲得しますが、失敗は現実です。提供された実行のキャプションには、1人のキャラクターのショットに2人の男性が表示されています。このチェックは、以前は正常として処理されていた欠陥を検出しました。**277 個のテスト**、CI は正常。完全な履歴は [CHANGELOG](CHANGELOG.md) にあります。

このリリースでは、[`fxdub.verify`](#the-public-api--fxdubverify) を追加しました。これは、宣言された、メディアに依存しないインターフェースであり、他のツールがその上に構築します。また、スイートのどのテストでも検出できなかった2つの欠陥を修正しました。1つは、作成されたファイルの絶対パスを含むコミットされたレシート、もう1つは、すべてのリリースで完全なマトリックスを2回実行する CI トリガーです。どちらも、実際の修正前のバイトに対して検証された検出器を備えています。
