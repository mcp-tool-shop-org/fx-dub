...
| | チェック項目 | 検出される問題点 |
|---|---|---|
| **`fxdub-receipt`** | 納品セット、48kHzのマスター音源、EBU R128ラウドネス基準、セリフとBGMの音量調整（ダッキング）の深さ、再多重化されたMP4ファイルには**両方の**音声トラックが含まれ、フレームは変更されず、**字幕に表示される人物数とキャストが一致しているか**。 | 無音の吹き替え、途中で終わる吹き替え、セリフがBGMに埋もれている、目標を外したミックス、**字幕担当者が存在しない人物を映像に追加してしまう**。 |
...
`on_frame`は、視覚情報がないパッケージでも字幕の欠陥を検出できるようにするものです。 `--scene`を`fxdub-receipt`に渡し、字幕が*主張する*人物数と、契約で表示されるべき人物数を比較します。納品された映像では、そのチェックに失敗します。字幕担当者は、一人しか映っていないシーンに対して*"二人の男性が向かい合っている"*という字幕を追加しており、この字幕が音声プロンプトとして使用されます。
...
`fxdub.vo_graphs`は、VOステージのグラフも作成します。具体的には、音声デザイン、同じエンジンを使用したオーディオ参照、クローンを作成して発声させる、スプライシング、タイムラインに配置する、ミックスなどです。また、映像ステージでは、フレーム抽出、リップシンク、多重化を行います。これらはすべて、代替手段（API JSONをチャットウィンドウに手動で入力する）を使用した場合、セッションとともにグラフが消えてしまい、すでに支払った欠陥が再び発生してしまうため、必要な処理です。

すべてのビルダーは、リポジトリのトラップ検出器によってLintチェックされます。そのため、実際の失敗につながる形状を誤って作成することはできません。その仕組みの2つの例：

- ElevenLabsのクローンノードの自動拡張入力は、実行時に`files.audio0`として扱われます。**決して**そのノード自身のスキーマに記載されている`files.item_1`としては扱われません。テスト実行では、誤った名前が問題なく受け入れられます。
- ByteDanceの`pitch_rate`はノード全体で有効であるため、一つのノードで異なるピッチで二人のキャラクターに音声を割り当てることはできません。タイムスタンプは絶対的な出力タイムラインを参照するため、修正は各キャラクターに対して一度ずつ行い、レイヤー化する必要があります。
- リップシンクノードの`speaker_selection`はデフォルトで*モデルに決定させる*設定になっています。この設定を変更しないと、ジョブが完了し、正しいフレーム数のMP4ファイルが適切な長さで出力され、すべてのコンテナチェックをパスしますが、誤った人物の口が動きます。ビルダーは座標を固定します。検出器は、その座標が固定されていないグラフに対して失敗します。
...
- **字幕にはタイミングではなく意味が含まれています。** 字幕を利用したパイプラインは、環境音とセリフの両方の品質に対応できます。字幕だけではドアの閉まる音を同期させることはできません。インパクトのあるタイミングを実現するには、イベントタイムラインが必要です。
...
- **シーンの説明はスクリプトではありません。** キャラクターが言うセリフを記述します。パイプラインはそのセリフを適切に聞こえるように調整します。
- **音声のアイデンティティは無料ではありません。** プロンプトで設計された音声は、*シードに関係なく*非決定的なものです。つまり、承認した音声を同じプロンプトを再実行しても再現することはできません。一度キャストし、承認されたオーディオを保存しておき、その後はそれを参照またはスプライシングして永久に使用します。異なるエンジンを使用したクローニングでも、アイデンティティは保持されません。これはリポジトリのトラップ台帳にある最も高価な教訓であり、検証者の`one_voice_per_character`チェックによって、その教訓が確実に守られます。
- **リップシンクは一つの顔を動かすため、一つのキャラクターのトラックが必要です。** 複数のキャラクターの音声をミックスして入力すると、画面に映っていない人物も含め、すべてのセリフが口パクで表示され、それでもすべてのオーディオチェックをパスします。なぜなら、オーディオ自体は変更されていないからです。各キャラクターごとに個別のトラックを入力すると、無音が正しいパフォーマンスになります。つまり、そのキャラクターはセリフを言わずに黙っています。結果は*シードに関係なく*非決定的なため、承認されたテイクは保存され、再レンダリングされることはありません。このノードはまた、映像のタイミングも調整します。納品物のフレーム数をチェックし、生の出力ではなく、それを基準に判断してください。
- **ミックスの数値は、規格とリスニング調査（BS.1770-5、AES TD1008、JAESダッキング研究）に基づいており、感覚的なものではありません。** また、それらは調整可能なノブであり、好みが測定可能に異なるためです。
- **ガバナンスは機能です。** 許可なく、実在の人物の声をクローンしないでください。EUで公開される合成音声には、Article 50に基づく機械可読形式のマーキング義務があります。レシートJSONは、そのプロベナンス（来歴）の一部となるように作成されており、[KBの公開セクション](docs/knowledge-base.md#publishing--governance-read-before-you-ship-a-dubbed-video)には、どこにどのような情報を開示する必要があるかが記載されています。特定の人物の声パックは絶対に作成しないでください。ロボコールにも使用しないでください。

## ステータス

**v1.1.0 — パイプラインが納品され、両方のレシートが緑色で、映像はリップシンクされています。** 二人のキャラクターが登場する夜の街並みのシーンでは、コンテナ契約（48 kHz、−18.09 LUFS、セリフ+11.17 LU over the bed、161フレーム変更なし、10.069秒）で**19/19点**、コンテンツ契約で**11/11点**を獲得しました。リップシンクされたバージョンは、同じ契約（832 × 480、161フレーム、両方のトラック）を満たしており、MACの口は自分のセリフを言い、画面外のキャラクターが話している間は閉じています。

`--scene`をパスすると、**19/20点**を獲得します。失敗の原因は明確です。納品された映像の字幕には、一人しか映っていないシーンで二人の男性が表示されています。このチェックは今回のリリースで新しく追加されたものであり、これまで検出されていなかった欠陥を発見しました。189回のテストすべてに合格し、CIも問題ありません。完全な履歴は[CHANGELOG](CHANGELOG.md)に記載されています。
