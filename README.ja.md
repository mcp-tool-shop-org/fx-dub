...
**v1.1.0 — パイプラインが納品され、両方のレシートが緑色で、映像はリップシンクされています。** 二人のキャラクターが登場する夜のストリートシーンでは、コンテナ契約（48 kHz、−18.09 LUFS、ダイアログ+11.17 LU over the bed、161フレーム変更なし、10.069 s）で**19/19点**、コンテンツ契約で**11/11点**を獲得しました。リップシンクされたバージョンは、同じ契約（832 × 480、161フレーム、両方のトラック）を満たしており、MACの口は自分のセリフを言い、オフフレームのキャラクターが話している間は閉じています。
