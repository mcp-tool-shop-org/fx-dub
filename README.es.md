...
|---|---|---|
| **`fxdub-receipt`** | conjunto de elementos entregables, masters a 48 kHz, sonoridad EBU R128, profundidad de atenuación del diálogo con respecto al fondo sonoro, el archivo MP4 re-multiplexado contiene **ambas** pistas, los fotogramas están intactos, **el recuento de personas en el subtítulo se compara con el elenco**. | un doblaje silencioso, un doblaje truncado, diálogo ahogado en el fondo sonoro, una mezcla que no alcanzó su objetivo, **un creador de subtítulos imaginando a alguien en la escena**. |
...
- La entrada de crecimiento automático del nodo de clonación de ElevenLabs se aborda como `files.audio0` en tiempo de ejecución, **no** como el `files.item_1` que anuncia su propio esquema, y una prueba preliminar acepta el nombre incorrecto sin quejarse.
- El `pitch_rate` de ByteDance es global para el nodo, por lo que un solo nodo no puede dar voz a dos personajes con tonos diferentes. Sus marcas de tiempo se refieren a una línea de tiempo de salida absoluta, por lo que la solución es realizar un único pase por personaje, en capas.
...
- **Los subtítulos transmiten significado, no tiempo.** Una cadena de procesamiento mediada por subtítulos es adecuada para ambientes y diálogos; nunca sincronizará un portazo solo con prosa. La sincronización de impacto requiere una línea de tiempo de eventos: el [Banco de conocimientos](docs/knowledge-base.md#stage-2b--direct-videoaudio-the-sync-first-alternative) mapea los modelos directos de video→audio que lo hacen de forma nativa, y sus licencias.
- **Una descripción de la escena no es un guion.** Escribe las palabras que dicen tus personajes; la cadena de procesamiento se encarga de que suenen bien.
- **La identidad vocal no es gratuita.** Las voces diseñadas mediante indicaciones no son deterministas *independientemente de la semilla*; una voz que apruebas no puede recuperarse volviendo a ejecutar la misma indicación. Define el elenco una vez, conserva el audio aprobado y luego utilízalo como referencia o empalmado para siempre. La clonación entre motores tampoco preserva la identidad. Esta es la lección más costosa en el registro de trampas del repositorio, y la comprobación `one_voice_per_character` del verificador es lo que garantiza que se aprenda.
- **La sincronización labial controla una cara, por lo que necesita la pista de un solo personaje.** Aliméntala con la mezcla y hará que cada línea se pronuncie, incluidas las que pertenecen a alguien que no está en la escena, y seguirá superando todas las comprobaciones de audio, porque el audio nunca cambió. Aliméntala con una pista por personaje y el silencio se convierte en la interpretación correcta: el personaje escucha. Los resultados no son deterministas *independientemente de la semilla*, por lo que se conserva la toma aprobada y nunca se vuelve a renderizar. El nodo también reajusta los tiempos de la imagen; verifica el recuento de fotogramas en el elemento entregable, no en su salida sin procesar.
...
- **La gobernanza es una característica.** No clones la voz de una persona real sin su consentimiento. El discurso sintético publicado en la UE conlleva una obligación de marcado legible por máquina del Artículo 50; el JSON de recepción está diseñado para formar parte de esa cadena de trazabilidad, y la [sección de publicación del Banco de conocimientos](docs/knowledge-base.md#publishing--governance-read-before-you-ship-a-dubbed-video) te indica qué divulgación debes hacer en el lugar donde publiques. No utilices paquetes de voz específicos de personas, nunca. Ni siquiera para llamadas automatizadas.
